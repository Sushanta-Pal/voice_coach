...
// Import Icons
import { SparklesIcon, CheckCircle, XCircle, BarChart, Annoyed, Wind, MessageSquareQuote, BookOpen, Repeat, BrainCircuit } from 'lucide-react';
...
const ScoreCard = ({ icon, title, score }) => (
    <div className="p-4 bg-slate-100 dark:bg-slate-800 rounded-lg text-center">
        ...
        <p className="text-3xl font-bold mt-2">{score}<span className="text-xl text-slate-400">/100</span></p>
    </div>
);
...
function FeedbackPage() {
    ...
    const communicationScores = isCommunicationSession ? feedbackData.feedback?.scores : null;
    ...
    return (
        <div className="container mx-auto max-w-5xl">
            ...
            <div className="text-center mb-12">
                <h1 className="text-4xl font-bold tracking-tight text-slate-900 dark:text-white">Performance Report</h1>
                <p className="text-lg text-slate-600 dark:text-slate-400 mt-2">An AI-powered breakdown of your {feedbackData.type} session.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                ...
                <div className="lg:col-span-3 space-y-8">
                    {!isCommunicationSession && (
                        <Card>
                            <CardHeader><CardTitle className="flex items-center gap-2"><MessageSquareQuote /> Your Answer</CardTitle></CardHeader>
                            <CardContent className="prose prose-slate dark:prose-invert max-w-none bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4">
                                <blockquote>"{feedbackData.answer}"</blockquote>
                            </CardContent>
                        </Card>
                            ...
                            <>
                                ...
                                {communicationScores && (
                                    <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-6">
                                        <ScoreCard icon={<BookOpen size={20}/>} title="Reading" score={communicationScores.reading} />
                                        <ScoreCard icon={<Repeat size={20}/>} title="Repetition" score={communicationScores.repetition} />
                                        <ScoreCard icon={<BrainCircuit size={20}/>} title="Comprehension" score={communicationScores.comprehension} />
                                    </CardContent>
                                )}
                                <CardHeader><CardTitle>Detailed Report</CardTitle></CardHeader>
                                <CardContent className="prose dark:prose-invert max-w-none">
                                    <div dangerouslySetInnerHTML={{ __html: feedbackData.feedback?.reportText.replace(/\n/g, '<br />') }} />
                                </CardContent>
                            </>
                        ) : (
                            <Tabs defaultValue="strengths" className="w-full">
